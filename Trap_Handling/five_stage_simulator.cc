...
void FiveStageSimulator::Execute() {
  ...
  if (op->inst_type == ECALL) {
    ...
    int64_t syscall_type = regs_[REG_A7]; 

    if (syscall_type == SYSCALL_FIND_MIN) {
      sepc_ = op->pc + 4;
      scause_ = 8;
      PipeRecover(kernel_trap_vector_); 
    }

  } else if (op->inst_type == SRET) {
    PipeRecover(sepc_); 

  } else if (IsBranch(op->inst_type) || IsJump(op->inst_type)) {
...
void FiveStageSimulator::MemoryAccess() {
  ...
  if (read_mem) {
    switch (mem_len) {
      case 1:
        if (read_sign_ext) {
          out = (int64_t)(int8_t)memory_->GetByte(out);
        } else {
          out = (uint64_t)(uint8_t)memory_->GetByte(out);
        }
        break;
      case 2:
        if (read_sign_ext) {
          out = (int64_t)(int16_t)memory_->GetShort(out);
        } else {
          out = (uint64_t)(uint16_t)memory_->GetShort(out);
        }
        break;
      case 4:
        if (read_sign_ext) {
          out = (int64_t)(int32_t)memory_->GetInt(out);
        } else {
          out = (uint64_t)(uint32_t)memory_->GetInt(out);
        }
        break;
      case 8:
